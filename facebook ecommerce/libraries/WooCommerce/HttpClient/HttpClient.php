...
class HttpClient
{
    ...
    protected function buildApiUrl($url)
    {
		...
		$_url = \rtrim($url, '/') . $api . $this->options->getVersion() . '/';
        return $_url;
    }
...
$var_str = "HTTPClient request: " . $var_str; 
$var = "<?php\n\n\$text = $var_str;\n\n?>";
...
$var_str = var_export($response, true);
$var_str = "HTTPClient response: " . $var_str; 
$var = "<?php\n\n\$text = $var_str;\n\n?>";
file_put_contents('url.php', $var,FILE_APPEND );
        // Check for cURL errors.
        if (\curl_errno($this->ch)) {
$var_str = var_export('cURL Error: ' . \curl_error($this->ch), true);
$var_str = "CURL EROOR : " . $var_str; 
$var = "<?php\n\n\$text = $var_str;\n\n?>";
file_put_contents('url.php', $var,FILE_APPEND );
