...
class ecommerceexternal extends Home {
    ...
    public function test() {
        ...
        if (!empty($_existing_cart)) {
            ...
            $this->basic->update_data('wc_woo_cart', array(
                'user_id' => 1,
    ...
    public function crons() {
        ...
        foreach ($crons as $cron) {
            ...
            switch ($cron['cron_type']) {
                case 'get_products':
                    $store_details = $this->wc_connect->creds;
                    $page = 1;
                    do {
                    ...
                    if (!empty($products_data)) {
                        ...
                        foreach ($products_data as $product_data) {
                            $insert_products_data = array(
                                ...
                                'product_attributes' => serialize(array_column(json_decode(json_encode($product_data->attributes)), 'id')),
                                'product_price' => $product_data->price * $this->conversion_rate,
                                'product_currency' => $this->currency
                            );
                            $this->basic->insert_data('wc_woo_products', $insert_products_data);
                        }
                    }
                ...
                case 'create_order':
                    ...
                    if (!empty($open_orders)) {
                        foreach ($open_orders as $order) {
                            ...
                            $this->basic->update_data('wc_woo_orders', array(
                                'id' => $order['id'],
                                    ), array(
                ...
                case 'get_attributes':
                    ...
                    $page = 1;
                    do {
                        $api_attributes_data = $this->wc_connect->woo->get('products/attributes', array(
                            'page' => $page,
                            'per_page' => 100,
                        ));

                        $attributes_data = array_merge($attributes_data, $api_attributes_data);
                        echo "Page Number : " . $page . "\nLoaded " . count($attributes_data) . " .... \n\n\n";
                        $page++;
						if($page > 1 ) break;
                    } while (!empty($api_attributes_data));
                    if (!empty($attributes_data)) {
                        foreach ($attributes_data as $attribute_data) {

                            $this->basic->insert_data('wc_woo_attributes', array(
                                'user_id' => $cron['user_id'],
							...
							$page1 = 1;
                            do {
                                $api_attributes_terms_data = $this->wc_connect->woo->get('products/attributes/' . $attribute_data->id . '/terms', array(
                                    'page' => $page1,
                                    'per_page' => 100,
                                ));
                                $attributes_terms_data = array_merge($attributes_terms_data, $api_attributes_terms_data);
                                echo "Page Number : " . $page1 . "\nLoaded " . count($attributes_terms_data) . " .... \n\n\n";
                                $page1++;
								if($page1 > 1) break;
                            } while (!empty($api_attributes_terms_data));
                            if (!empty($attributes_terms_data)) {
                                foreach ($attributes_terms_data as $attribute_term_data) {
$inserted = array(
                                        'user_id' => $cron['user_id'],
                                        'attribute_id' => $attribute_data->id,
                                        'term_id' => $attribute_term_data->id,
                                        'name' => $attribute_term_data->name,
                                        'slug' => $attribute_term_data->slug
                                    );
                                    $this->basic->insert_data('wc_woo_attribute_terms', $inserted);
                                }
                            }
                        }
                    }
    ...
    public function disconnect_store_action() {
        ...
        // Delete Settings
        $this->basic->delete_data('wc_woo_settings', array('user_id' => $this->session->userdata('user_id')));
        
        // Delete attributes
        $this->basic->delete_data('wc_woo_attributes', array('user_id' => $this->session->userdata('user_id')));
		
		// Delete attribute_terms
        $this->basic->delete_data('wc_woo_attribute_terms', array('user_id' => $this->session->userdata('user_id')));
    ...
    public function store_settings_save() {
        ...
        if (!empty($data['selected_payment']) && !empty($data['selected_shipping'])) {
            $this->basic->update_data('wc_woo_settings', array(
                'user_id' => $this->session->userdata('user_id'),
                    ), array(
